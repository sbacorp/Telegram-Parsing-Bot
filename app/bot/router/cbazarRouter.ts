...
countMaxAds.use(
	async (ctx: Context) => await ctx.reply({ reply_markup: cancel })
);
...
getUrls.on("message:text", async (ctx: Context) => {
	...
	for (var i = 0; i < urls.length; i++) {
		...
		if (!found) return ctx.reply("*Неверно, повторите попытку*");
	}
...
startingSbazar.on("message:text", async (ctx: Context) => {
	if (ctx.msg.text === "Начать парсинг") {
		const values = {
			productsCount: Number(ctx.session.countMaxAds),
			daysAgo: Number(ctx.session.publishDate),
			year: 2022 - Number(ctx.session.registrationDate),
			count: ctx.session.countOutput,
		};
	...
	if (ctx.msg.text === "отмена") {
		await ctx.reply("*Остановлено*", { reply_markup: mainMenu });
		ctx.session.sbazarStep = "countMaxAds";
		return;
	}
});
